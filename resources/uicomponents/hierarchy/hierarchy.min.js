...
function(b){a(this).find(".dropdown-menu > .breadcrumb-tree").each(function(){if(!a.jstree.reference(a(this)))a(this).xtree().one("ready.jstree",function(e,c){e=c.instance;(c=e.element.attr("data-openTo"))&&e.openTo(c)})})}).children(".dropdown-menu").on("click",function(b){b.stopPropagation()})};a(document).on("xwiki:dom:updated",function(d,b){d=a(b.elements);d.is(".breadcrumb")&&g(d)});g(a("ol.breadcrumb"))})}).apply("",[{"web.hierarchy.error":"Не удалось получить полную иерархию."}]);
