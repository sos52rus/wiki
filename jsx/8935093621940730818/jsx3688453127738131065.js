...
XWiki.Annotation = Class.create({
  ...
  initialize : function (displayHighlighted, annotatedElt, displayedByDefault) {
    ...
    if (!this.annotatedElement) {
      ...
      if (displayedByDefault) {
        new XWiki.widgets.Notification("Не удалось загрузить аннотации, поскольку контент недоступен.", 'warning');
      }
      return;
    }
    ...
    if (window.location.hash === '#edit' || window.location.hash === '#translate') {
      ...
      if (XWiki.docsyntax != 'xwiki/1.0') {
        ...
        new XWiki.widgets.Notification("Аннотации недоступны для страниц, написанных с использованием синтаксиса XWiki / 1.0.", 'warning');
      }
    }
  },
  ...
  toggleSettingsPanel : function(event) {
    ...
    if (!this.settingsPanel) {
      new Ajax.Request('http://xwiki.st.tech/bin/view/AnnotationCode/Settings?xpage=plain', {
        parameters : {'target' : XWiki.currentWiki + ':' + XWiki.currentSpace + '.' + XWiki.currentPage},
        onCreate: function() {
          // disable the button
          menu.disabled = true;
          // show nice loading message at page bottom
          menu._x_notification = new XWiki.widgets.Notification("Загрузка настроек аннотаций", 'inprogress');
        },
        ...
        onFailure: function(response) {
          ...
          menu._x_notification.replace(new XWiki.widgets.Notification("Неудача:" + failureReason, 'error', {timeout : 5}));
        },
  ...
  fetchAnnotations : function(andShow, force) {
    require(['xwiki-meta'], function (xm) {
      ...
      new Ajax.Request(getAnnotationsURL, {method: 'GET',
        ...
        onCreate: function() {
          // show nice loading message at page bottom
          this._x_notification = new XWiki.widgets.Notification("Загружается документ с аннотациями", 'inprogress');
        }.bind(this),
        ...
        onFailure: function(response) {
          var failureReason = response.statusText || 'Server not responding';
          // show the error message at the bottom
          this._x_notification.replace(new XWiki.widgets.Notification("Неудача:" + failureReason, 'error', {timeout : 5}));
  ...
  checkResponseCodeAndFail : function(response) {
    if (response.responseJSON && response.responseJSON.responseCode != null && response.responseJSON.responseCode == 0) {
      ...
      if (response.responseJSON) {
        ...
        response.statusText = "Неправильно отформатированный ответ сервера";
      }
      response.request.options.onFailure(response);
      return true;
    }
    ...
    item.observe('click', function(event) {
      ...
      if (item.disabled) {
        ...
        new XWiki.widgets.ConfirmedAjaxRequest(
          ...
          {
             confirmationText: "Вы уверены, что хотите удалить эту аннотацию?",
             progressMessageText : "Удаление аннотации...",
             successMessageText : "Аннотация удалена",
             failureMessageText : "Не удалось удалить аннотацию:"
          }
        );
      }
    }.bindAsEventListener(this));
  ...
  addValidateListener : function(item, id, container, inBubble) {
    item.observe('click', function(event) {
      ...
      this.updateAnnotationAsync(container, id, inBubble, item.href, 'POST',
        ...
        {
          successText : "Аннотация подтверждена.",
          failureText : "Ошибка:"
        });
    }.bindAsEventListener(this));
  },
    ...
    require(['xwiki-meta'], function (xm) {
      new Ajax.Request(XWiki.currentDocument.getURL('get'), {
        ...
        onFailure: function(response) {
          ...
          // show the error message at the bottom
          this._x_notification = new XWiki.widgets.Notification("Ошибка:" + failureReason, 'error', {timeout : 5});
        }.bind(this),
  ...
  onAnnotationEdit : function(event, container, annotationId, inBubble) {
    ...
    this.updateAnnotationAsync(container, annotationId, inBubble, form.action, form.method, formData,
      {
        successText : "Аннотация обновлена.",
        failureText : "Ошибка:"
      });
  },
  ...
  onAddAnnotationShortcut : function() {
    ...
    if (!selectionText) {
      ...
      new XWiki.widgets.Notification("Пожалуйста, выберите непустой текст в содержимом страницы.", 'error', {timeout : 5});
    } else {
  ...
  onAnnotationAdd : function(event) {
    ...
    new Ajax.Request(form.action, {
      ...
      onSuccess : function (response) {
        ...
        this.fetchedAnnotations = true;
        form._x_notification = new XWiki.widgets.Notification("Аннотация добавлена.", 'done');
...
require.config({
  paths: {
    'fast-diff': "..\/..\/webjars\/fast-diff\/1.3.0\/diff"
  }
})
